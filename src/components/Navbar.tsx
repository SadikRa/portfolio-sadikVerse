...
const Navbar = ({ session }: { session: UserProps | null }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-background/80 backdrop-blur-md border-b border-border shadow-sm z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 py-4">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2">
          <img
            ...
            className="shadow-lg border-4 border-gray-200 dark:border-gray-700 rounded-full"
          />
          <span className="text-2xl sm:text-3xl font-extrabold text-primary hover:scale-105 transition-transform">
            SadikVerse
          </span>
        </Link>
        ...
        {/* Desktop Menu */}
        <ul className="hidden md:flex space-x-6 text-foreground font-medium">
          {["Home", "Projects", "Blog", "Contact", "Dashboard"].map((item) => (
            <li key={item}>
              <Link
        ...
        <div className="flex items-center space-x-4">
          ...
          {session?.user ? (
            <Button
              onClick={() => signOut()}
              className="hidden sm:inline-block px-4 py-2 rounded-lg bg-red-500 text-white hover:bg-red-600 transition"
            >
              LogOut
            </Button>
          ) : (
            <Link
              href="/login"
              className="hidden sm:inline-block px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition"
            >
              Login
            </Link>
          )}
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-background/95 backdrop-blur-lg border-t border-border shadow-lg">
          <ul className="flex flex-col space-y-4 px-6 py-6 text-foreground font-medium">
            {["Home", "Projects", "Blog", "Contact", "Dashboard"].map((item) => (
              <li key={item}>
                <Link
                  href={`/${item.toLowerCase() === "home" ? "" : item.toLowerCase()}`}
            ...
            <li>
              {session?.user ? (
                ...
                <Link
                  href="/login"
                  onClick={toggleMobileMenu}
                  className="block px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition"
                >
                  Login
                </Link>
              )}
